...
test('zombie give-up: an issue at the attempt cap never gets assignIssue/rerunIssue, logs zombie_give_up, sets blocked, and gets a best-effort comment', async () => {
  ...
  const stuckIssue = makeIssue({ project_id: AURIGA, status: 'in_progress', assignee_id: 'A', parent_issue_id: 'fake-parent' });
...
test('zombie assign: an unassigned in_progress zombie gets assignIssue then rerunIssue (PANT-409)', async () => {
  ...
  const stuckIssue = makeIssue({ project_id: AURIGA, status: 'in_progress', assignee_id: null, parent_issue_id: 'fake-parent' });
  const { backlog, spawn, calls, runsByIdentifier } = createMockAdapters([stuckIssue], cfg.AGENTS);
...
test('zombie rerun: per-cycle-per-agent cap is enforced — second rerun gets zombie_skip (PANT-576)', async () => {
  ...
  const zombie1 = makeIssue({ project_id: AURIGA, status: 'in_progress', assignee_id: aurigaBuildId, parent_issue_id: 'fake-parent' });
  const zombie2 = makeIssue({ project_id: AURIGA, status: 'in_progress', assignee_id: aurigaBuildId, parent_issue_id: 'fake-parent' });
...
test('zombie rerun: updates priorAgentCycleAssigns, blocking picks-loop double-dispatch (PANT-576)', async () => {
  ...
  const stale = Date.now() - (60 * 60 * 1000);
  const zombieIssue = makeIssue({ project_id: 'zombie-picks-proj-576', status: 'in_progress', assignee_id: aurigaBuildId, parent_issue_id: 'fake-parent' });
...
test('zombie rerun: updates loopRtProjected, blocking zombie-assign over-dispatch on same runtime (PANT-576)', async () => {
  ...
  const rerunZombie = makeIssue({ project_id: 'zombie-rt-proj-576', status: 'in_progress', assignee_id: aurigaDevId, parent_issue_id: 'fake-parent' });
  // Zombie assign: no assignee → action:'assign' → calls chooseAgentForProject.
  const assignZombie = makeIssue({ project_id: 'zombie-rt-proj-576', status: 'in_progress', assignee_id: null, parent_issue_id: 'fake-parent' });
