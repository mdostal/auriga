...
test('detectZombies: in_progress with no runs -> assign (no assignee) / rerun (assignee)', () => {
  const now = Date.now();
  const inProgress = [
    { id: 'z1', identifier: 'z1', project_id: 'MINERVA', status: 'in_progress', assignee_id: null, title: 'stalled', parent_issue_id: 'ep1' },
    { id: 'z2', identifier: 'z2', project_id: 'AURIGA', status: 'in_progress', assignee_id: 'A', title: 'stalled', parent_issue_id: 'ep1' },
    { id: 'z3', identifier: 'z3', project_id: 'AURIGA', status: 'in_progress', assignee_id: 'A', title: 'healthy', parent_issue_id: 'ep1' },
  ];
  const runs = {
    z1: [],
...
test('detectZombies: stale-but-old run triggers recovery, fresh done does not', () => {
  ...
  const inProgress = [
    { id: 'z4', identifier: 'z4', project_id: 'AURIGA', status: 'in_progress', assignee_id: 'A', title: 'stale', parent_issue_id: 'ep1' },
    { id: 'z5', identifier: 'z5', project_id: 'AURIGA', status: 'in_progress', assignee_id: 'A', title: 'recent', parent_issue_id: 'ep1' },
  ];
  const runs = {
...
test('detectZombies: run count below zombieMaxAttempts still emits assign/rerun (no regression)', () => {
  ...
  const inProgress = [
    { id: 'g1', identifier: 'g1', project_id: 'AURIGA', status: 'in_progress', assignee_id: null, title: 'stalled', parent_issue_id: 'ep1' },
    { id: 'g2', identifier: 'g2', project_id: 'AURIGA', status: 'in_progress', assignee_id: 'A', title: 'stalled', parent_issue_id: 'ep1' },
  ];
  const runs = {
    g1: [
...
test('detectZombies: run count at/above zombieMaxAttempts emits give-up instead of assign/rerun', () => {
  ...
  const inProgress = [
    { id: 'g3', identifier: 'g3', project_id: 'AURIGA', status: 'in_progress', assignee_id: 'A', title: 'stuck at cap', parent_issue_id: 'ep1' },
    { id: 'g4', identifier: 'g4', project_id: 'AURIGA', status: 'in_progress', assignee_id: null, title: 'stuck above cap', parent_issue_id: 'ep1' },
  ];
  const runs = {
...
test('detectZombies flags isHive on the zombie action so re-routing respects HIVE_LANE', () => {
  const now = Date.now();
  const inProgress = [
    { id: 'z6', identifier: 'z6', project_id: 'AURIGA', status: 'in_progress', assignee_id: null, title: 'stalled hive story', description: HIVE_DESCRIPTION, parent_issue_id: 'ep1' },
  ];
...
test('detectZombies skips agent-parked issues (isAgentParked guard)', () => {
  const now = Date.now();
  const inProgress = [
    { id: 'zp1', identifier: 'zp1', project_id: 'AURIGA', status: 'in_progress', assignee_id: 'A', title: 'parked', metadata: { blocked_reason: 'waiting for human approval' }, parent_issue_id: 'ep1' },
    { id: 'zp2', identifier: 'zp2', project_id: 'AURIGA', status: 'in_progress', assignee_id: 'A', title: 'not parked', metadata: {}, parent_issue_id: 'ep1' },
  ];
...
test('detectZombies skips human-todo in_progress issues regardless of staleness or attempt count', () => {
  ...
  const inProgress = [
    ...
    { id: 'ag1', identifier: 'ag1', project_id: 'AURIGA', status: 'in_progress', assignee_id: 'A', title: 'agent work', labels: [], metadata: {}, parent_issue_id: 'ep1' },
  ];
...
test('detectZombies skips seed issues (isSeed guard — PANT-594)', () => {
  const now = Date.now();
  const seedIssue = { id: 'seed1', identifier: 'seed1', project_id: 'AURIGA', status: 'in_progress', assignee_id: null, title: 'plan something', labels: ['needs-plan'], metadata: {} };
  const nonSeed = { id: 'ns1', identifier: 'ns1', project_id: 'AURIGA', status: 'in_progress', assignee_id: 'A', title: 'build thing', labels: [], metadata: {}, parent_issue_id: 'epic1' };
  const allIssues = [seedIssue, nonSeed];
  const z = core.detectZombies([seedIssue, nonSeed], { seed1: [], ns1: [] }, CFG, now, allIssues);
  const ids = z.map((a) => a.identifier);
  assert.ok(!ids.includes('seed1'), 'seed issue must be skipped by isSeed guard');
  assert.ok(ids.includes('ns1'), 'non-seed stale issue must still be recovered');
});
...
test('detectZombies reroutes a hive story stuck on a codex lane instead of rerunning it', () => {
  ...
  const hiveOnCodex = { id: 'z1', identifier: 'PAN-z1', project_id: 'MEM', title: '[m-05-x] y', status: 'in_progress', assignee_id: 'A', description: HIVE_DESCRIPTION, parent_issue_id: 'ep1' };
  ...
  const plainOnCodex = { id: 'z2', identifier: 'PAN-z2', project_id: 'AURIGA', title: 'plain task', status: 'in_progress', assignee_id: 'A', parent_issue_id: 'ep1' };
