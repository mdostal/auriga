...
export function detectZombies(inProgressIssues, runsByIssue, cfg, now = Date.now(), allIssues = []) {
  ...
  for (const i of inProgressIssues) {
    ...
    // PANT-636: count only build-phase runs for the give-up threshold — review-lane
    // runs inflate the counter and cause premature give-up on legitimately retried stories.
